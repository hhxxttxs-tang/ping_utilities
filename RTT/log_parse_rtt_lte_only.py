...
icmp_pattern = seq(
    ...
    icmp_delta = (skip_inline_until_after(r't=') >> parse_current(r'\d+') << skip_until_EOL) | success('-99')
).many()
...
for _, row in df_id_filter.iterrows():
    ...
    if cur_icmp_seq != dict_row['icmp_seq']:
        ...
        # if (lte_ul and lte_dl) or (icmp_delta and icmp_delta != '-99'):
        # delta_eth = eth_dl*1000 - eth_ul*1000
        delta_lte = None
        if (lte_ul and lte_dl):
            print("lte_ul and lte_dl exists!!!")
            delta_lte = lte_dl*1000 - lte_ul*1000
        elif (icmp_delta_lte and icmp_delta_lte != '-99'):
            print("icmp_delta_lte != '-99!!!!")
            delta_lte = float(icmp_delta_lte)

        delta_eth = None
        if (eth_ul and eth_dl):
            print("eth_ul and eth_dl exists!!!")
            delta_eth = eth_dl*1000 - eth_ul*1000
        elif (icmp_delta_eth and icmp_delta_eth != '-99'):
            print("icmp_delta_eth != '-99!!!!")
            delta_eth = float(icmp_delta_eth)

        delta_cpe = None
        if delta_lte and delta_eth:
            delta_cpe = delta_eth - delta_lte

        new_row =  ['61515',cur_icmp_seq, eth_ul, lte_ul, lte_dl, eth_dl, delta_eth, delta_lte,delta_cpe]
        print("add new row to DF", new_row)
        s = pd.Series(new_row, index=df_final.columns)
    ...
    if dict_row['icmp_interface'] == 'lte-dl':
        ...
        icmp_delta_lte = dict_row['icmp_delta']
    if dict_row['icmp_interface'] == 'eth-dl':
        eth_dl = dict_row['icmp_tm']
        icmp_delta_eth = dict_row['icmp_delta']
...
print(df_final["delta-lte"].dropna().astype(float).describe())

print("\nSummary statistics of delta-cpe-a:")
print(df_final["delta-cpe-a"].dropna().astype(float).describe())

df_rtt_original = pd.read_csv('/Users/ezhou/Documents/smawave/tech_support/cases/011_APMT_longbeach/RTT/0821/hrping.csv')
df_rtt = df_rtt_original['RTT'].apply(lambda x : float(x[:-2]))
print("\nSummary statistics of ping record:")
print(df_rtt.describe())
